...
| [accuracy_audit.py](../../scripts/accuracy_audit.py) | Compares Genie answers to ground-truth SQL. | Inherited (Pulse) | **Broken** — hardcoded host `dbc-f88d29ce-4aa2.cloud.databricks.com` and warehouse ID `6510da50329f1e85` ([accuracy_audit.py:16-17](../../scripts/accuracy_audit.py#L16-L17)), reads `proxy/config.json["profiles"]["sales"]["token"]` which only exists if you copy the example config verbatim. |
