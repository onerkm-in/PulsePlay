...
### 2. Per-user / per-profile rate limit (security-agent HIGH)
**Why deferred:** today `rateLimitMiddleware` runs **before** `idpMiddleware` (`app.use('/assistant', rateLimitMiddleware, idpMiddleware)`), so `req.user` isn't set at rate-limit time. Per-user limiting needs a **second middleware placed after idp across all 7 cost-bearing prefixes**, plus updating the BUG-015 mount-introspection invariant. That's real surgery on the auth/rate-limit chain — it should have human eyes, not an autonomous edit.
**Plan:** add `perUserRateLimitMiddleware` (bucket by `req.user.sub`, generous limit e.g. 60/min/user, no-op when unauthenticated) mounted right after `idpMiddleware` on the same prefix list; extend `tests/server.test.js` BUG-015 to assert it's mounted; +behaviour tests (35 req/min from one user → 429; other users unaffected). Not urgent — no multi-user pilot yet.
