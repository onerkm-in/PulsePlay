...
What does the **reasoning**. 10 backend paths today; the dispatcher pattern makes adding more cheap.

**Current state (DONE):**
- **10 backend paths** in `proxy/server.js` — Genie, Azure OpenAI chat, Azure OpenAI analytics, Foundation Model, Bedrock-direct, Bedrock-RAG, Supervisor, Supervisor-local, ResponsesAgent, and Power BI semantic-model deterministic DAX, with the shared dispatcher layer above them
...
**Non-Databricks proof point:**
Same "Power BI + Azure OpenAI" deployment. Among the 10 backend paths is Azure OpenAI; the Prompt IR translates equally to OpenAI prompts. Pulse Insights pipeline, Discovery Loop, and Frame-to-prompt all work without a single Databricks call.
