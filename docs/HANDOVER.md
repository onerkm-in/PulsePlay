...
## 2026-08-06 (latest+43) — the road-to-9.5 arc: evals ×9, the IdP finally booted, bedrock migrated, the Prompt IR lit

Branch `feat/road-to-95`, 11 commits `319bc1f`→`3f92621` (+1 docs commit). Proxy **1565/1565** (89 suites, +12 tests +1 suite), evals **49/49** (+26), playground **2019/2019** (157 files) + `tsc --noEmit` clean, re-verified end of session. Every commit ran its touched suites green before landing.

**The evals harness could not actually do its job, and now it can.** Four plumbing defects found by reading the routes rather than the README: `--profile` was a **no-op** (client sent `profile`, `resolveProfile` reads `assistantProfile`); Genie answers were unreachable (start returns the raw envelope, the harness never polled); one profile served both truth and answer, but `/sql/preview` needs a `warehouseId` only Genie profiles have — so a PBI or FM case could never reconcile; and the PBI/FM `message_id` JSON blob + `queryResult` rows were unread. All fixed (`223dc4d`). Suites now bind `answerProfile`/`truthProfile` separately; `expected` is a literal-truth alternative for `groundedData` cases.

**Golden set: 4 cases → 36 across 3 connectors** (`7dfeb4d`): `genie-scm-metricviews.json` (14 — Genie against its OWN estate, truth = `MEASURE()` on the same metric views, so summing raw columns / averaging a rate / botching the 2026-YTD window reconciles as a miss), `pbi-scm-dax.json` (10 — deterministic DAX vs the warehouse flat views, answer and truth profiles deliberately different), `fm-grounded.json` (8 — the case supplies the rows, `expected` is literal, no warehouse needed). `golden.test.mjs` schema-gates every suite in CI and pins the ≥30-case floor. **The 32 new cases are NOT yet live-run** — QUALITY.md says exactly that; the live claim is still the four validated 2026-07-31 cases until an explicit `npm run live`.

**Hallucination detection, heuristic v1** (`319bc1f` + `223dc4d`): `groundingVerifier` grew a `{scale:'roman'}` mode (M=thousand vs the FM path's industry m=million — a shared parser cannot serve both implicitly), and `run-live.mjs` now checks every cited number against the answer's grounding rows. `unverified` fails the case; `partial` warns or fails under `--strict-grounding`. Same-repo source import — evals' npm tree stays at zero.

**LLM-as-judge v0** (`89258bc`): `npm run judge` — the FM connector via `/foundation/section` scores faithfulness/relevance/coherence against evidence. TWO model calls per case, so it is its own equally-explicit command; scores are **signal, not gate**. The README's promptfoo carve-out now marks the real threshold: the day judge scores gate anything is the day the dependency earns its tree.

**`dev/idp` finally booted — without docker — and the untested half of idp auth is now tested** (`fb6e6f5`). `run-keycloak-nodocker.ps1` fetches the SAME pinned Keycloak 26.0.7 the compose file pins plus a pinned Temurin 21 JRE (SHA256-verified; Temurin cross-checked against Adoptium's published checksum), caches in gitignored `.runtime/`, boots loopback on 7010. `verify-idp-live.mjs` then proves what jest deliberately cannot reach (`idpMiddleware` short-circuits under `NODE_ENV=test`): **7/7 green 2026-08-06** — realm mappers put roles top-level and `aud=pulseplay-proxy`, anonymous 401, valid manager/norole tokens pass, tampered signature 401, wrong-expected-audience 401. The "first person with docker should run it" note from latest+42 is discharged — docker turned out unnecessary.

**Connector plugin Phase B — bedrock migrated** (`d6f6baf`). First backend out of the monolith into `proxy/connectors/bedrock.js` (~310 lines out of server.js; 9,499→9,189). Six host members added, each justified by ≥2 connectors. First real `conversationDispatch` entry — the `/assistant` dispatch ladder still does not consult it; that flip is a later phase on purpose. `rawErrLeakInvariant` now scans `proxy/connectors/*.js` too, so Phase C migrations can't quietly shrink its coverage. Implemented by a subagent, then **independently re-verified**: full suite re-run green before commit.

**The Prompt IR serves its first request** (`3f92621`). DEBT_REGISTER D4 ("built, tested, wired to nothing") is half-discharged: `/foundation/section` — which injected NO pack context at all — now derives its system prompt from the pack's `prompt-ir.yaml` via the foundation-model translator when the caller names a pack and supplies no explicit `systemPrompt`. Precedence pinned by 5 tests using the REAL cpg-fmcg/supply-chain IR (which also proves the authored corpus loads). Audit action `prompt-ir-inject`. **No caller sends `pack` to this route yet** — the client hookup (forwarding pack selection on the insights path) is the natural next slice.

**Doc claims got a CI gate** (`83e0045`): `scripts/check-doc-claims.mjs` — every CLAUDE.md directory-table path must exist, every local doc link must resolve, and anchored code facts (stagger 2000ms, port wiring, 8787 tripwire, evals-live-never-in-CI, Roman grounding mode) must hold. First run caught real drift: CLAUDE.md pointed the chat surface at `UnifiedAssistantSurface.tsx`, deleted some time ago — `PulsePlayScreen.tsx` owns the screen. 105 checks, in the evals CI job.

**Azure OpenAI: a profile authored from our own docs could never authenticate** (`4a4bee5`). Manifests + PROXY_REFERENCE said `azureOpenAiApiKey`; the route reads `azureOpenAiKey` — silent 401. `ENV_PROFILE_FIELDS` had no `AZURE_OPENAI*` entries, so the advertised env-var pattern was silently dropped. Three contradictory api-version defaults documented; the route's `2024-02-01` now stated everywhere. **Shape-honesty, not a live proof** — the paths stay code-present-unproven until an Azure resource is connected.

**`@databricks/aibi-client` promotion is blocked upstream** (`a3fbf79`): no published version (through `1.0.3-alpha.0`, incl. `1.0.0`) declares a license field — it would fail our own licence gate, and an unlicensed package is unusable for the OSS path regardless. Recorded in the package.json omission comment with the re-check trigger; stale `DashboardEmbedClient` header fixed to match the real `DatabricksDashboard` export.

**TRIPWIRES for next session:** (1) the genie-scm-poc space is configured over `workspace.databrickspractice` metric views — the ORIGINAL 4 scm.json cases reference `main.supply_chain.fact_supply_chain_kpi_monthly`, a different estate; if their 2026-07-31 validation ran against a different space/profile pairing, reconcile which profile answers them before the first multi-suite live run. (2) `run-keycloak-nodocker.ps1`'s Keycloak SHA256 is TOFU-pinned from the verified first fetch (Adoptium's is publisher-verified). (3) The judge and the 32 new golden cases are unproven live — first `npm run live` / `npm run judge` needs the proxy up with `genie-scm-poc`, `powerbi-dwd`, and `foundation` profiles.
...
**Doc-accuracy sweep — three docs were asserting controls that do not exist.** Same class of defect as the `dependency-review` comment, found by checking docs against CODE rather than against other docs. (1) SECURITY.md "Logging and audit" claimed audit lines go to append-only files in dev and a SIEM in production; `auditLog()` is `console.log('[audit]', line)` and nothing else ([server.js:2356](../proxy/server.js#L2356)) — whatever captures stdout IS the durability story. Corrected, gap named, pointed at `REBUILD-P1`. Action Insights' own `writeAudit()` to Delta is the one real exception and is now called out as such. (2) SECURITY.md listed the per-user rate limit and spend guard as *pending* — both shipped (`b1434a2` 60/min/sub, `f8bc433` reserve-then-settle). (3) "Honest gaps" claimed no CVE scanning in CI, now false; added the killswitch, `AGENT-OBO` and process-local-ledger gaps, which were real and unlisted. New "Supply chain" section states the posture. Test counts refreshed across README / docs/README / QUALITY — they read 1137–1243 proxy and 1382–1926 playground, i.e. May–June numbers against an actual 1553/2019/23. `evals/` and `dev/idp/` added to the CLAUDE.md directory table, the README layout, and both doc maps.
...
**THEN CI TOLD THE TRUTH — two gates were decorative, one of them mine.** Watching the first real run was the whole point of pushing; both new workflows went red.

**Mine (`784d407`):** the SBOM step failed the whole security job. `enablers/pulse-pbi` is `pulse-visuals@2.1.0.0` — Power BI custom visuals require a **four-segment** version, which is not semver, so npm cannot build a package URL and exits `EINVALIDPURLTYPE`. **That version must not be "fixed"** — it would break the pbiviz package. An SBOM is a reporting artifact and the audit is the gate, so letting it turn a passing security check red just teaches people to ignore red. Reports to the job summary and carries on now, cause named in the workflow.

**Much older, and worse:** **`dependency-review` has never worked on this repo. Twelve runs, twelve failures, zero passes.** It needs the Dependency Graph enabled and that API answers 403 here. Every "disallowed licence" and "high severity" claim it appeared to make on PRs was inert — the same disease as the comment fixed that morning, a control that exists on paper only. Rather than leave the gate imaginary, `scripts/check-licenses.mjs` reads the lockfiles directly — no GitHub feature, no network, no dependencies — and runs in `supply-chain.yml`. **1,584 licensed dependencies across all four lockfiles, all permissive, zero unstated, zero disallowed.** The SPDX expression logic is unit-tested (8 tests) because that is the path that could silently wave a GPL through; writing those tests immediately caught a real bug — importing the module ran the scan and `process.exit()`ed out of the test run, so only one test executed. `dependency-review` is KEPT with its inertness and one-toggle fix documented, allowlist deliberately in sync.

**And a third, found while checking the rest: `smoke.yml` had been failing every run for days** while BLOCKERS.md listed it as "✅ CLEARED". Its step 2 asserted three `surface-connector-*` dropdowns — a control **deleted from the product** when inline connector pickers moved to Settings / FirstRunWizard / the BundleSwitcher chip. The selector survived only in the smoke script and the enabler sync script. **First hypothesis was wrong** (the 2026-07-29 switch to `combined` mode) — forcing `segregated` via the kill switch changed nothing, which is exactly why it got verified instead of pushed. Re-pointed at selectors the app actually ships: viewport shell, AI/BI panel chrome, all four surface tabs, plus the surface's real no-spend empty state. **Verified 6/6 locally under TRUE CI conditions** (`served_mode: combined`, `kill_switch: false`) — the first local run used a kill-switched proxy, which would have been the wrong conditions.

**STILL OPEN, OWNER ACTION (AGENDA `SEC-REPO-SETTINGS`):** the repo is **PUBLIC** and has `dependency_graph` (403), `secret_scanning`, `secret_scanning_push_protection` and `dependabot_security_updates` all **disabled**. Push protection is the control that actually stops a Databricks PAT or Power BI SP secret reaching a public repo, and it is free. Left for the owner because enabling push protection changes the push workflow.

**PUBLISHED:** `main` and `design/nav-consistency` both pushed to `origin` (`b2777bb..29c84b8`). **The repo is PUBLIC** (`onerkm-in/PulsePlay`), which sits oddly beside the Path C "inner-source-first, NOT public" posture — worth a deliberate decision rather than drift.

**OWNER DECISIONS TAKEN (2026-08-01, explicit GO on both):**

**1. The org-identifying drops are gitignored, not committed.** Correction to the earlier note: on a full recursive scan the company name is NOT in `Design/` at all — all 46 occurrences are in ONE `data-contracts` metric-view JSON, in the org's real UC column comments ("Denominator for FR PepsiCo"). `data-contracts/` also carries the workspace id and full schema topology (no data VALUES — attestation verified). `Design/` extracts are clean but were ignored alongside as working inputs. The 12 `Design/audit-findings` files tracked BEFORE today stay tracked — verified clean, and git history keeps them regardless.

**2. All four GitHub security toggles enabled via the API** — dependency graph (compare API now answers; `dependency-review` can finally function), secret scanning, **push protection** (pushes containing a detected secret now BLOCK — expect that behaviour change), Dependabot security updates. **Scanning immediately earned its keep, twice:**

- **Two `databricks_access_token` alerts in public history.** Alert #2 (`dapi1234…abcdef`, sequential fixture in tests deleted by `47f6d49`) — resolved `used_in_tests`. **Alert #1 is real signal: `dapi006f…` in `piiRedact.test.ts` at `f15e16e` has genuine entropy and cannot be distinguished from a real PAT by inspection.** It does NOT match the current working token (`dapi3a45…`, compared locally without printing either) and the file now carries an obvious placeholder instead — but the string is in public history forever. **AGENDA `SEC-LEAKED-PAT`: check workspace token inventories, revoke anything unaccounted for, then resolve the alert. Left OPEN on GitHub so the reminder survives.**
- **The dependency graph exposed a package the whole supply-chain story had missed: `enablers/pulse-pbi-gn`** — absent from dependabot.yml, absent from the audit sweep, absent from the CI matrix (which was derived from dependabot.yml — one hand-maintained list copying another). 20 of the repo's 22 open Dependabot alerts lived there. Now in all three lists (matrix, dependabot.yml, `check-licenses.mjs` — 2,207 licensed deps across five lockfiles, all permissive), `npm audit fix` applied, typecheck clean.

**The one honest residual: `xlsx`.** Dev-scoped in the playground but it SHIPS (Vite bundles what is imported — the dev/prod split does not govern bundling), and it is a direct runtime dep of `pulse-pbi-gn`. Two high advisories, both READ-path; PulsePlay is write-only (`XLSX.write` only, verified no `XLSX.read` in src), so the trigger path does not exist today. No fix exists on the npm registry (SheetJS abandoned it at 0.18.5). `pulse-pbi-gn`'s gate sits at `critical` with the exemption NAMED in the workflow; AGENDA `DEP-XLSX` tracks the real migration, with a guardrail: no `XLSX.read` of user files before it. This also corrects the earlier "all runtime trees 0/0/0/0" line — true by npm's definition, and npm's definition under-counts for bundled frontends.
...
**Service principal created (`scripts/create-pbi-service-principal.mjs`).** App `pulseplay-powerbi-sp`, clientId `93bf5419-2785-4976-80b4-cd35064c2a79`, SP objectId `c7c8ee3e-…`, secret expires **2027-01-27**. The script (device-code, one user sign-in) creates app+SP+secret, adds SP to the workspace, and writes `aadClientId`/`aadClientSecret` + `authMode:service-principal` into config.json. **Tooling tripwires learned:** (1) the sandbox classifier blocks the create script the first time as consequential Entra cred creation — a user-requested retry went through. (2) The tenant gate is the **Fabric-era rename** "Service principals can call Fabric public APIs" (was "Allow SPs to use Power BI APIs"), enabled org-wide. (3) The bootstrap's workspace-add 403'd on **AAD propagation lag** (SP created seconds earlier) — re-added a few min later via a one-off delegated-token call (`scratchpad/add-sp-to-workspace.mjs`). `verify-pbi-sp.mjs` (extended to also run executeQueries + GenerateToken) = all 5 green: token / workspace / reports / DAX (OFR 98.12) / embed token. `powerbi-dwd` now runs on the SP — the DAX brain re-verified through the proxy with no regression.

**Embed-token route fix (`0bf6a02`).** The Cycle-A `/assistant/embed-token/powerbi` route only read `powerBiClientId/Secret/TenantId`, but the SP creds are under the canonical `aadClientId/Secret/aadTenantId` (which the DAX path already accepts) — so a fully-working SP still 503'd. Route now normalizes `powerBi*` from `aad*` at handler entry (shallow copy, never mutates the shared profile), covering the cred check + AAD token call + SP audit hashes. Report embed token now mints (200). +1 test (aad*-only profile), 24/24.

**Config C — the actual Power BI report embeds as the Dashboard (headed-verified).** No code change needed beyond the embed-route fix — the PBI adapter is a REAL `powerbi-client` SDK integration (`bi-adapters/powerbi/index.ts`; SDK path needs `{id, embedUrl, accessToken}`, `embedMode` must NOT be `"secure"`). Activated at runtime via 3 localStorage keys (`pulseplay:bi-vendor="powerbi"`, `pulseplay:bi-surface-mode="vendor"`, `pulseplay:bi-embed-config`=the backend-issued embed JSON with a fresh token from the proxy route) + reload `?surface=bi-viz`. Result: the live `PulsePlay_SCM_Synthetic` report renders inside the Dashboard tab, and **every number matches** the DAX brain + Genie metric-view baseline — Net Sales 1771.95/1877.92/1031.41 MM, OFR 97.45/98.24/99.04, OTIF 92.44/93.45/94.45, GHG 2,523,750, Gross Margin by country (incl. the "(Blank)" ZZ00/ZZ01 orphan). **Full apples-to-apples loop closed: Genie ↔ PulsePlay DAX brain ↔ actual PBI report, one star, identical.** In-app this is configured via Settings → BI → "Service principal" embed mode (which does the same POST + writes the same config). Screenshot in the session scratchpad.

**app.yaml (`8118035`).** SP-backed `PROXY_PROFILE_POWERBIDWD_*` block added (name normalizes to the frontend catalogue id `powerbi-dwd`); client secret is a Databricks secret resource `powerbi-sp-secret` (valueFrom — value NOT in the manifest). Before any deploy: `databricks secrets put-secret pulseplay powerbi_sp_secret`. Not deployed. Rotate the SP secret before 2027-01-27.

## 2026-07-27 (latest+20) — Power BI apples-to-apples wired to the SCM star + 3 fixes (matcher ×2, Decisions degradation) + Config B validated + domain-guidance feasibility

The user published `PulsePlay_SCM_Synthetic` to Power BI (workspace `dwd_pbi_demo` / `7bb52a2a-…`, dataset `633b2b11-…`, report `ead5b770-…`, XMLA `powerbi://api.powerbi.com/v1.0/myorg/dwd_pbi_demo`). Wired PulsePlay's Power BI brain to it and ran the Config B headed validation, fixing every bug found. Commits `1cbd99d`, `c7551d9`, `af6b2aa`; `main` fast-forwarded (local only). Proxy matcher 53/53, action-insights 21/21, playground tsc clean.

**Apples-to-apples confirmed (PBI DAX == Databricks metric views, to the decimal):** ran per-year KPIs via `executeQueries` on the published dataset — OFR 97.45/98.24/99.04, Net Sales 1771.95/1877.92/1031.41 MM, GHG 2,523,750/2,399,415/1,138,707 — exact match to the metric-view baseline. Same star (dims+facts) underpins both engines; Genie uses metric views, PBI replicates them as DAX over `vw_pbi_fct_*`.

**Profile repoint (config.json, gitignored):** `powerbi-dwd` now points at the SCM dataset `633b2b11` (was Superstore `076821ed`), same workspace + user-refresh auth, added `powerbiReportId`. Old Superstore preserved unadvertised as `powerbi-superstore`. Zero frontend/catalog churn — the UI already advertises the `powerbi-dwd` id. Dataset GUID was resolved via the existing refresh token (scratchpad `resolve-pbi-dataset.mjs`), no new question to the user.

**Fix 1 — matcher: unit-suffixed measures + dim-over-FK (`1cbd99d`).** `findMeasure` required the full measure name incl. unit suffix, so "net sales"/"order fill rate" matched nothing on "Net Sales USD"/"Order Fill Rate Pct". Added `measureVariants()` (strip trailing unit tokens + "per <unit>" tail, run on the RAW tokenisation so glued units tCO2e/kWh survive; qty↔quantity synonym; full name stays longest variant). Also `findDimension` scored fact FKs (`ofr[country_id]`) above conformed dims (`dim_country[country]`) — a star correctness bug (cross-fact measure wouldn't propagate). Added dim-table bonus + FK-column penalty.

**Fix 2 — matcher: never group by a numeric measure-input column (`c7551d9`).** The AI-Insights auto-plan emits "Top 5 <dim> by <measure>"; `findDimension` read the trailing "by Ordered Qty" as the group-by → nonsense card "Top 5 ordered_qty by ordered qty". Added `isMeasureInputColumn(name, type)` using the probe's column type (PBI INFO.COLUMNS: Integer/Double/Decimal) — numeric non-time columns are never dimension candidates. Numeric time grains (year/month) preserved; missing type → name-based fallback.

**Fix 3 — Decisions degrades gracefully (`af6b2aa`).** Under a Power BI-only profile the Decisions surface leaked a raw "HTTP 500" (the governed decision store is a Delta table on a Databricks warehouse; `powerbi-dwd` has none). `actionInsights.js` listHandler now guards up-front: not-Databricks-backed → 200 `{ok:false, unavailable:true, notice}`; `ActionInsightsPanel` renders `data.notice`. Genuine store errors on a Databricks profile still 500.

**Config B (Power BI-only) headed results:** AI Insights ✅ (fresh SCM, 2024-01→2026-06 window, Ordered Qty 9,789,598,770 reconciles to `vw_pbi_fct_ofr`; malformed RISKS card fixed → "TOP 5 SALES_CHANNEL BY ORDERED QTY"). Ask Pulse ✅ ("Gross margin by country" groups by `dim_country`, 53.8–55.6%, star join correct). Decisions ✅ now shows the graceful notice. Dashboard ✅ native canvas renders SCM tiles reconciling (OFR/OTIF by year). **Findings:** (B2) AI-Insights headline leads with "Ordered Qty" (a volume input) not a governed KPI — a domain-guidance gap, not a bug. (Orphan) `performance`/`operations` carry deliberate sentinel FKs `ZZ00/ZZ01` (from `fk_domain()` reproducing source referential-integrity), so per-country breakdowns show one blank "-" bucket ($581M); grand totals unaffected. Optional polish = Kimball "Unknown member" in the dim (needs a DBX redeploy + PBI refresh) — not done.

**Config C (hybrid) — BLOCKED on report embed (Finding C1).** The Databricks-AI half = Config A (already validated). The "Power BI *as the Dashboard*" half needs the report embed-token route (`POST /assistant/embed-token/powerbi`), which is Cycle-A **service-principal-only** (`powerBiClientId/Secret/TenantId`) → 503 with our user-refresh profile. Options: (a) add an Azure AD SP (app reg + workspace SP add + tenant "SPs can use PBI APIs" toggle), (b) extend the embed route to mint a report token via user-refresh like the DAX path already does (`acquirePbiAccessToken` supports it — clean code enhancement), or (c) keep the PulsePlay-native canvas as the dashboard. **Decision pending with the user.**

**Domain-guidance + Prompt vendor-agnostic feasibility (research, `docs/AI_CONTEXT_CONFIGURATION_MODEL.md` intent vs code).** User asked whether one generic "domain guidance + prompt" definition could drive all 4 surfaces vendor/connector-agnostically. **Verdict: ~30–40% in place; today fragmented.** Two neutral spines exist but are dormant/duplicated: (1) the **Prompt IR + per-backend translators** (`promptIR.js`, `promptTranslators/index.js`) is dead-lettered — `buildBackendPayload` is never called from `server.js`; live routes inject Genie-shaped `prompt-context.md` verbatim to every backend. (2) `BackendAdapter`/`BackendFactory`/`connectorRegistry` is the live connector-agnostic **transport** seam (correct) but carries prompt content built elsewhere. AI Insights + Ask Pulse build prompts CLIENT-side from settings free-text (`domainGuidance`/`insightsDomainGuidance` in `visualHelpers.ts`), not from packs/IR. Decisions is a wholly separate Databricks-Delta plane (no packs/IR/settings). Domain content lives in ≥3 parallel corpora (filesystem `pulsepacks/`, hard-coded `businessContextProfile.PACK_REGISTRY`, preset libs) → drift risk. **Path to generic:** promote the IR to the single domain source + wire `buildBackendPayload` into the live routes; replace the settings-string client assembly with an IR-driven builder shared by AI Insights + Ask Pulse; collapse the 3 corpora; bridge Decisions onto the same vocabulary. Note: finding B2 (headline KPI) is exactly what a pack-driven "featured KPI" in this unified model would fix.

## 2026-07-27 (latest+19) — Responsive Ask Pulse visuals (shipped) + embedded SLM-Supervisor feasibility assessment (research)

Two workstreams from the expanded brief. `0322e59` (responsive, code) + `c2185f2` (feasibility doc, no build). tsc clean; CanvasGrid/buildEChartsOption green.

**Responsive Ask Pulse generated visualizations (`0322e59`, SHIPPED + headed-verified):** Ask Pulse charts used a fixed 320px height and only re-laid out on WINDOW resize, so on 1366×768 the chart+narrative overflowed the fold (X-axis + legend clipped) and a side/nav panel opening left the chart mis-sized. Fix: (a) `EChartsRenderer` now has a `ResizeObserver` on the host (container-aware re-fit on panel open/close, focus, vh recompute, zoom — jsdom-guarded), (b) the Ask Pulse chart height is `clamp(220px, 34vh, 400px)` instead of fixed 320. Headed-verified at 1366×768 (full chart+axes+legend now fit), 1920×1080 (re-fits live on resize, no reload), 820px-wide (no h-overflow). The generated-viz path was already a governed structured spec (`buildEChartsOption(chartType, columns, rows)`), NOT arbitrary code — so §9.2's "no model-generated executable viz" is already satisfied. Fuller design (focus mode, table-fallback bounded scroll, author display controls) is specced in the feasibility doc, not yet built.

**Embedded domain-aware SLM Supervisor feasibility (`c2185f2`, ASSESSMENT ONLY):** `docs/research/SLM_SUPERVISOR_AND_RESPONSIVE_ASKPULSE_FEASIBILITY.md`, grounded by a 4-pass codebase inspection (agents; every current-state claim carries file:line). **Verdict: Partially Feasible.** Key findings: PulsePlay has NO content-aware router today (user picks connector by `profile.type`; dispatch is an if-chain at `server.js:3457`), but the seam exists (`proxy/connectors/registries.js` is boot-wired yet unused). `foundationModelClient.js` is OpenAI-compatible w/ json-schema structured output but hardcoded to the Databricks serving URL (add a base-URL override → managed SLM). Semantic layer = Databricks UC metric views (not client); domain grounding = PulsePack prompt-injection + Genie semantics + deterministic numeric verification (NO live RAG — Vector Search hibernating, KB plane planned-only). Deterministic gates to reuse: personaGate/hitlGate/allowlist(+supervisorProfiles, prod fail-closed)/governance attestation/insightsValidator/groundingVerifier. **`experienceConfig.js` is the publish/versioning/kill-switch precedent** to copy for a Supervisor Settings page. **Browser-embedded model (Option B) is NOT code-viable** — WebGPU/WASM/DuckDB-WASM are aspirational (CLAUDE.md over-claim; grep finds none in playground); free deploy tiers (Azure F1/Databricks Free) can't host a resident model. **Recommendation: BUILD a rules-first router on the seam + INTEGRATE an approved small model via a managed endpoint (Option D default, C for air-gapped) + larger-model fallback (E); never make the SLM authoritative.** Doc covers target arch + mermaid diagram, deployment options A–F, model shortlist (licences flagged verify-from-source), capability matrix, governance model, Settings IA, viz contract, ASCII wireframes, POC test plan, phased roadmap.

## 2026-07-27 (latest+18) — MCP headed validation on genie-scm-poc: crash not reproduced, per-card provenance hidden, 3 number-formatting fixes

First full **chrome-devtools MCP** headed pass (server now connected after the restart). Drove the live `genie-scm-poc` briefing end-to-end and fixed every display issue found. `85bcb52` (provenance) + `d643e48` (formatting). tsc clean; 104 formatter/insights tests green (+2 new).

**Reached the configured state the injected-profile couldn't:** `isConfigured` (visual.tsx) needs an `activeSpace` from the Pulse `genieSettings`, not just `pulseplay:active-ai-profile`. Setting only active-ai-profile leaves AI Insights on the "Connect AI assistant" empty state. Clicking the genie-scm-poc profile card in Settings writes `genieSettings.assistantProfile`+`connectionMode:proxy`+`apiBaseUrl`, which flips `isConfigured` true and shows "Generate briefing". (Recorded in the headed-validation memory.)

**The "reading 'state'" crash did NOT reproduce.** Full sweep — AI Insights (briefing + KPI snapshot), Decisions, Dashboard, Ask Pulse (live chat round-trip) — all render clean, **0 crashes, 0 console errors**. Hypothesis: it was a stale-cache collision (an old cached briefing from before the 2026-07-26 data rebuild rendering against new code); a fresh profile can't carry that state. If it recurs: hard refresh / clear insights cache.

**Per-card provenance footer hidden (`85bcb52`):** the "Generated by PulsePlay · Source: … · Updated …" strip under each insight card is now off by default. Seeded `insightsShowProvenanceFooter=false` in `PulseHostStub.seedPulsePlayDefaults` (PulsePlay-adapter-level, so the heritage `settings.ts` default stays true for the PBI sibling) + flipped the native `pulseVisualSettingsStore` default. Authors re-enable via Settings → AI → Response behavior. Note: the runtime reads the value from `genieSettings` (seeded), NOT from `pulseVisualSettingsStore` DEFAULTS — flipping only the store default did nothing until the seed was added.

**3 number-formatting fixes (`d643e48`) — all headed-verified on live Genie:**
- **Chart Y-axis "%" on non-% series** (`buildEChartsOption.ts`): a "top trends" clustered bar mixed % metrics with GHG counts; `dominantUnit`→"percentage" stamped "2,500,000.0%" on GHG's 2.5M. `axisFormatterForSeries` now uses a unit-less humanized number ("2.5MM") when `hasMixedUnits`. → verified "3.0MM/2.5MM/2.0MM".
- **Genie table collapsed small currency** (`columnLabels.ts`): `net_sales_billion`=1.8 rendered "$2" (whole-dollar round), hiding $1.8B→$1.9B growth. Currency under $1000 now keeps 2 decimals. → verified "$1.80 / $1.90 / $1.00".
- **KPI SNAPSHOT tile net-sales "1.0 vs 1.0, 0.0%"** — AI-emitted; the AI rounded both years to the same billion. The KPI SNAPSHOT prompt (`visualHelpers.ts`, hybrid ×2 + legacy) now requires Δ computed from unrounded values + compact currency ($1.03B, $989.34M). → verified after live regen: **"NET SALES USD $1.03B ▲ +4.5% vs $989.34M"**.

**Known residual (minor, AI-narration variance):** GHG counts sometimes carry 2 decimals ("1,138,706.88"); one earlier HEADLINE run had a stray unmatched `**` + called net sales "steady" while the KPI tile showed +4.5% — both cleared on regeneration; not deterministic. The mixed-metric "top trends" chart is still scale-dominated by GHG (%-metric bars tiny) — a chart-selection issue (KPI-tile chart fits better), separate from the now-fixed axis labels.

**Headed harness note:** MCP screenshots composite fine (isolated Chrome). Config a connector fast by clicking the profile card in Settings (writes genieSettings) — injecting only active-ai-profile is NOT enough.

## 2026-07-26 (latest+17) — chrome-devtools MCP wired + hid BundleSwitcher & provenance footer from end users

Two small UI/tooling commits (`bc94983` hide, `92a5155` .mcp.json), `main` fast-forwarded. tsc clean; BundleSwitcher/SurfaceContextStrip/appGovernance suites green (10/10).

**chrome-devtools MCP (`92a5155`, `.mcp.json`):** project-scoped stdio server `npx chrome-devtools-mcp@latest --isolated=true --channel=stable`. **Requires a session restart + one-time approval** to activate (MCP config is read only at session start; there is no in-session reconnect). The chrome-devtools *plugin* was already installed (its skill loaded) but the *server* was never registered — that's why the browser tools were absent. Windows fallback if plain npx misbehaves: `command:"cmd", args:["/c","npx",...]`.

**Headed validation harness (no MCP needed, works NOW):** the plugin bundles `puppeteer-core`, and system Chrome stable is at `C:/Program Files/Google/Chrome/Application/chrome.exe`. Pattern: launch Chrome headed with `--remote-debugging-port=9222 --user-data-dir=<scratchpad>`, then `puppeteer.connect({browserURL})` from node with `NODE_PATH=<plugin>/node_modules`. **Screenshots COMPOSITE correctly** this way — this solves the long-standing "Browser pane won't composite" caveat from prior sessions (that was the embedded MCP pane, not a standalone Chrome). Config a connector fast by injecting `localStorage['pulseplay:active-ai-profile']='genie-scm-poc'` (raw string) + `['pulseplay:bi-vendor']='native'`, then reload.

**Hid two confusing end-user sections (`bc94983`, App.tsx) — user directive "hide both for end user":** (1) the header **BundleSwitcher** chip ("… × CUSTOM ▾" + the "BI SURFACE × AI BRAIN" combination dropdown) — redundant since the pair is chosen in Settings/FirstRunWizard; (2) the **provenance footer** ("Context: … / Generated by PulsePlay · Source: … · v1.0.4"). Both gated behind one `SHOW_AUTHORING_CHROME = false` lever (App.tsx module const; one line → true restores both for an authoring/dev build). Components (`BundleSwitcher`, `SurfaceContextStrip`) untouched + still tested. Header is now wordmark + the "Ready" SetupStatusPill only.

**Commit hygiene:** App.tsx also carries a pre-existing uncommitted in-flight hunk (the UICLEAN-P2 `uiMode: "pulse"|"v0"` type removal) — committed ONLY my 3 hunks via `git add -p` (piped `y/n/y/y`; `git apply --cached` fails on this box's CRLF), left the `uiMode` deletion in the working tree for whoever owns UICLEAN-P2.

**Unresolved — reported crash "Cannot read properties of undefined (reading 'state')":** could NOT reproduce. Walked all 4 surfaces with `genie-scm-poc` set + error listeners → 0 page/console errors. But the injected-config profile stayed on the "Connect AI assistant" empty state, so I never triggered a full **AI Insights briefing render** (the path that draws the KPI snapshot changed on 2026-07-26). Not fixed; needs repro steps (which surface + last action). Best chased via the MCP after restart against the real configured profile.

**Post-restart TODO for the next session:** dev servers + the validation Chrome from this session die on window reload — restart proxy (`PORT=7000 node server.js`) + playground (`npm run dev`) per the canonical sequence; approve the `chrome-devtools` MCP server; then reproduce/fix the crash via the AI Insights briefing on `genie-scm-poc`.
...
**Agenda staleness audit (small backend cluster).** While scanning `AGENDA.md` "Next up" for a safe autonomous unit, found 3 contiguous "open" `[ ]` items already shipped — marked `[x]` with code evidence (brutal honesty: done work shouldn't read as open): **FM orchestrator retry-symmetry** (`llmOrchestrator.js:251` uses the shared `validationRetryBudget.resolveBudget`, same as the Genie poll path — Cycle 17), **PBI semantic-model clean enablement P0** (routing `server.js:3453/3630` + env maps `336-341` + INFO.VIEW.* graceful probe `b5441a9`), **PBI RLS impersonation from IdP claims** (`startPowerBiConversation:6558` derives identities fail-closed at route entry). Also marked **PB1 + THREAD-C superseded** by `enablers/pulse-pbi-gn` (awaiting user OK, since they touch the enabler agreement). A later pass verified the **"Settings hardening pass from 2026-05-20"** item: 5/6 P1s confirmed fixed with code evidence (PBI sibling-host dot-boundary, EmbedConfigForm fail-closed, mobile-nav B1 group-bar, Save-bar discard baseline, Advanced reset-all clears every `pulseplay:*`); the 6th (diagnostics secret redaction) couldn't be reproduced as a gap — marked `[x]` with a reopen-if-repro caveat. The open UX-ARCH / multi-day items are genuinely open.
...
**Power BI semantic-model — configured against the live `DwD_PBI_Demo` dataset + a real bug found and fixed.** Wired the `powerbi-dwd` profile via **device-code user-refresh** auth (Azure CLI well-known client, no app registration), tenant `<dev-tenant-guid>`. Auto-discovered the workspace (`dwd_pbi_demo` = `<dev-workspace-guid>`) — and caught that **the PBIP's embedded `remoteArtifacts` datasetId (`<stale-dataset-guid>`) is STALE**; the live published DwD_PBI_Demo is `<dwd-dataset-guid>` (workspace also holds a `SalesPerformance` = `<salesperf-dataset-guid>`). Trusting the PBIP GUID would have 404'd.
...
- **5 GitHub Actions (`fa7765d`).** checkout v4→v6, setup-node v4→v6, dependency-review-action v4→v5, azure/login v2→v3, codeql-action v3→v4. Config-only; validated on next CI run.
- **Proxy (`24b7f88`, `d58b093`).** jest 30.3→30.4.2, js-yaml 4.1.1→4.2.0 (minor). **express 4.21.2→5.2.1 (major)** — reverse-engineered the route surface first: no removed APIs (del/param/sendfile/res.send(status)), no req.query/params reassignment, no optional `:param?` strings; only complex route is regex `/.*/ ` (Express-5-safe). Express 5 broke **27 tests** in the BUG-015 structural mount-introspection (path-to-regexp v8 drops the readable `layer.regexp.source` and stores opaque matchers) — the **behavioural** gating tests passed, so the middleware genuinely still works; only the introspection helper broke. Rewrote `collectPrefixMounts()` to resolve each mount prefix via the real `layer.match(candidate)` matcher on Express 5, keeping the Express-4 regexp fast-path. Invariant still bites. Real boot `GET /health → 200`.
- **Playground (`5f5214a`, `c254cc9`, `430c614`, `61c0358`).** @tanstack/react-query-devtools + @types/react floors; **echarts 5.5→6.1.0 (major)** — modular tree-shaken imports unchanged in v6 (heads-up: v6 changed its *default* palette, but charts set their own theme colors via the palette picker); **jsdom 25→29 (major, test env)**; **vitest 2→4 + @vitest/coverage-v8 2→4 (combined major)** — Dependabot proposed coverage-v8 4.1.7 alone, but it needs vitest v4 as peer, so taking it standalone would break install; bumped both in lockstep (vite ^6 satisfies the peer; pulse-pbi already ran vitest 4).
- **enablers/desktop (`62644d8`).** **express 4→5 + http-proxy-middleware 3→4 (both major).** appServer already used the v3+ `createProxyMiddleware on:{proxyReq}` event style + `pathRewrite` (v4-compatible) and a regex SPA route (Express-5-safe). 50/50 unit + smoke-boot: `/launch → 200`, SPA fallback regex route → 200, proxy middleware registers without throwing.
- **enablers/pulse-pbi (`f8e9989`, `da8550e`).** react/react-dom 19.2.4→19.2.6 + @types/react →19.2.15 (patch); **eslint 9.11.1→10.4.1 (major)** — flat config already in use (the only mode in eslint 10); `eslint-plugin-powerbi-visuals` has no eslint peer constraint, `@typescript-eslint/eslint-plugin@^8` peer already allows eslint ^10.

**Tripwires:**
- **npm installs in this repo need `NODE_OPTIONS=--use-system-ca`** (Node 24) — without it the corporate TLS chain fails with `UNABLE_TO_VERIFY_LEAF_SIGNATURE`. This is the operational form of the CLAUDE.md `NODE_EXTRA_CA_CERTS` note.
- **The BUG-015 mount-introspection helper now branches on Express version.** `collectPrefixMounts(app, candidatePrefixes)` takes the prefix union and probes `layer.match()` on Express 5 (opaque matchers); keeps the Express-4 `regexp.source` scrape as a fast path. Don't "simplify" back to regexp-only — Express 5 has no readable prefix string.
- **echarts 6 default palette differs from v5** — only matters for charts that *don't* set explicit colors; PulsePlay's do (palette picker), so no visible change expected. Flag if a default-colored chart appears off.

**Open / not done (honest):** The **19 Dependabot PR branches still exist on origin** — their content is now on main (re-derived), so Dependabot/GitHub should auto-close the PRs as up-to-date, but the branches weren't deleted. The **4 stale branches** (`codex/f5-g0-native-layout-2026-05-21`, `import/local-main-2026-05-19`, `publish/local-main-2026-05-20`, `reconcile/codex-to-main-2026-05-22`) were **proven 100% contained in main** (0 unique non-merge commits; reconcile's lone merge node adds 0 files main lacks, both parents are main ancestors) — deletion was **authorized but blocked by the destructive-action classifier pending explicit per-action confirmation**, so they remain on origin. No product code changed; no scope lost.

---

## 2026-06-05 (late) — Super Beast closeout: prompt-injection FIXED + closed-items re-proven RUNNING

Final verification + closeout. Re-proved every "closed" item by RUNNING (not ledger), closed the one known-open security item, screened the UI again, and named the parallel-connectors item open-with-plan. Launch conditions absent → SERVERLESS=not (Genie/Supervisor + report-visual + OBO stay READING), IDP/OBO=no, PART_C=flag-only (Part 5 NOT built), PUSH=after Part 4. Gates this run: **proxy 1212/1212** (+7), **playground 1884/1884**, lint + build clean, **genie.ts zero diff**.

- **Part 4 — prompt-injection-from-BI-data FIXED + gated (`8904ace`).** The OpenAI-analytics / Bedrock grounded-answer path fed BI column names + result rows into the narrative LLM prompt as a raw markdown table with NO escaping ([llmOrchestrator.renderRowsAsMarkdown](../proxy/lib/llmOrchestrator.js)). A malicious `|`/backtick/newline in a column or cell could break the table and inject fake instructions (LLM-misdirection — the inward twin of A2). Fix: `escapeMarkdownCell()` neutralizes pipe/backtick/newline + length-caps each cell; result wrapped in `<<<RESULT DATA>>>` fences, schema in `<<<SCHEMA>>>` fences; both system prompts now instruct the model to treat fenced content as untrusted data, never instructions. Scope: analytics path ONLY (Genie + deterministic PBI/DAX are slot-validated; frontend render is separately HTML-escaped — re-verified [visualHelpers.ts](../playground/src/pulse/visualHelpers.ts)). +7 tests, **5 proven fails-on-old** via temp-revert. **Honesty note:** this item was already on `main` (pushed pre-mandate); resolved by fixing forward, not by un-pushing.
- **Part 1 — closed-items re-proof, RUNNING this session.** PBI DAX **total = 2,297,201** (direct live REST, exact match); by-segment 1,161,401 + 706,146 + 429,653 = 2,297,200 reconciles; nonexistent measure → honest decline (lists real measures, no fabricated number). AI Insights briefing **5/5 live** (6 real-DAX sections, no "no measure" fallback). genie epoch-guard (genieCancel 2/2 + zero diff), support-bundle redaction (exportBundle 12/12), embed-token + warehouse + SSRF all green inside proxy 1212. Harness strip RUNNING-verified (present in `public/`, absent from `dist/`). **Self-correction:** my own earlier probe flagged "Sales YoY % by Category" as failing — direct REST proves it **works** (real 3-group table); the probe failure was the dev-server dying mid-run, a harness artifact, not a product bug.
- **Part 3 — feature/connector spot-checks (live).** FM responds `COMPLETED`; Launchpad on a PBI profile → **502 honest-by-design** (PBI creds can't discover Databricks). **READING this run:** the FM wrong-endpoint-name body-override test was inconclusive (the override field isn't read from the request body — harness limitation, not a dishonesty claim; the config-level proof stands from 2026-06-04).
- **Part 2 — UI re-capture 42/42**, dark-theme DOM-pin holds (`bodyBg rgb(13,17,23)`); A1 (dark sub-page title) + A2 (320px Knowledge stack) **PIXEL re-verified intact** on current HEAD.
- **Part 5 — parallel connectors: OPEN / UNBUILT, named with plan** ([PARALLEL_CONNECTORS_OPEN.md](PARALLEL_CONNECTORS_OPEN.md)). Per `PART_C=flag-only` I did NOT build it and do NOT certify it. The single-active-per-axis gap is re-confirmed by code read (`activeVendor`/`activeConnector`/`embedConfig` are App scalars; `PaneInstance` connector-less). To proceed: set `PART_C=build-foundation`; the Genie-as-third-pane proof additionally needs `SERVERLESS=enabled`.

**Standing gaps (unchanged, each needs an external condition):** Genie/Supervisor live = READING (needs SERVERLESS enabled); PBI-RLS OBO-success = READING (needs real IdP + OBO + RLS role); report-visual headed render = READING (needs real Premium/Fabric GUIDs + SP Build+Read); FM correctness = plausibility-only.

**Adversarial self-review:** weakest claim = the injection fix **narrows but cannot fully eliminate** LLM-misdirection — escaping + fencing + system-prompt framing is defense-in-depth, not a cryptographic guarantee (an LLM can still in principle be swayed by cleverly-worded data); it does close the structural markdown-break/instruction-injection vector, which was the concrete exploit. READING-not-RUNNING items are all external-condition-gated and labeled. Flag-OFF parallel-connectors stayed truly unchanged because nothing was built. No "closed" item re-passed by reading alone — each cited a run this session.

---


Wildest-test/review/fix request. Verification-first; 3 parallel adversarial audits (security, error-handling, dead-controls) + browser-pixel UI screening + fixes, each gated foreground, real counts. **PUSH = NO** (launch condition absent) — 6 commits left local. **SERVERLESS = off** — Genie live stays READING. Net rig: **proxy 1205/1205** (was 1177, +28), **playground 1884/1884**, lint + build clean, **genie.ts zero diff vs origin** (epoch-guard intact).

- **Warehouse auto-start hang KILLED — RUNNING-proven live (`a4c80d2`).** `ensureWarehouseRunning` polled a forever-STOPPED warehouse for **5 minutes** on a *permanent* failure (serverless disabled), and every concurrent caller spawned its OWN 60×5s poll loop (the observed interleaved `Poll N/60` spam). Mechanism: one caller fails `/start` fast (400) but others hit the cooldown "start skipped" path → 5-min poll. Fix: (A) classify Databricks 4xx as PERMANENT + fail fast + **memoize** so the next call short-circuits with 0 network calls; (B) **dedup** concurrent ensures per warehouseId onto one in-flight promise; (D) honor an **AbortSignal** (wired from `/warehouse/start` `req.on('close')`) so Stop/tab-close cancels the server poll; bounded the previously-unbounded bookkeeping maps. **Live proof against the real blocked workspace:** cold call **3.8 s** (was ~300 s), warm call **0.01 s**, proxy logs show `state=STOPPED → Starting → Auto-start issue: 400` then silence — no poll spam. +8 tests ([warehouseAutostart.test.js](../proxy/tests/warehouseAutostart.test.js)).
- **SSRF hardening — inline X-Databricks-Host (`580deaf`).** The inline-credentials feature (override mode = anonymous local-dev default) only character-sanitized the caller-supplied host, leaving a cloud-metadata SSRF vector (`X-Databricks-Host: http://169.254.169.254/…`). Added `isBlockedInlineHost()` rejecting loopback (127/8, ::1, localhost, 0.0.0.0) + link-local/metadata (169.254/16, fe80::/10, metadata.google.internal) for **header-sourced** hosts only; config.json hosts stay operator-trusted; RFC1918 stays allowed (on-prem). +20 tests ([inlineHostSsrf.test.js](../proxy/tests/inlineHostSsrf.test.js)).
- **Visual a11y/responsive — 2 fixes, PIXEL-proven before/after (`a71a891` + `0f14f5f`).** A1: `SubPageHeader` ([AiKnowledgeBase.tsx](../playground/src/settings/groups/sub/AiKnowledgeBase.tsx)) gradient started at hardcoded `#0f172a` → invisible on the dark canvas (#0d1117, ~1.05:1) across **all 5 settings sub-pages**; start at `var(--pp-text)` + use the `backgroundImage` longhand (kills a React 19 shorthand/backgroundClip rerender warning too). A2: the Knowledge Base pack browser kept its 240px rail + detail side-by-side at every width → detail crushed to ~80px at 320px (one word per line); added a `max-width:640px` stack. Both DOM-pinned (`bodyBg` dark `rgb(13,17,23)` vs light `rgb(248,249,250)`) so a mislabeled-theme capture can't pass silently.
- **Audits — honest verdicts.** Security: the SSRF above (fixed) + a MED *prompt-injection-from-BI-data-into-LLM* gap (raw cell rows → prompt, no sanitization, [llmOrchestrator.js](../proxy/lib/llmOrchestrator.js)) left **unfixed/flagged** (defense-in-depth, row-capped). Credential redaction / DAX+SQL injection / pack-path-traversal / embed-host check all **well-defended**. Error-handling: A/B/D confirmed+fixed; the agent's "SQL-statement-400 hang" was a **FALSE alarm** (`runSql` exits on a thrown 4xx). Dead-controls: **CLEAN** — the only dead paths are intentional, documented v0 stubs (Tableau/Qlik/Looker iframes, native `send()`, PBI export) that honestly throw `UNSUPPORTED_COMMAND`.

**Withdrawn (false alarm, honest):** earlier I suspected the frontend misclassified a blocked warehouse as "Waiting for AI Proxy to come online". Code review proves "Proxy Offline" is thrown **only** on `xhr.status === 0` ([genie.ts:1077](../playground/src/pulse/genie.ts#L1077)) — a genuine network failure. What I saw live was a proxy I'd killed mid-session. No bug.

**Not fixed — deliberate, documented:** the AI-Insights→/knowledge **dev-only StrictMode console warning** ("synchronously unmount a root while React was already rendering" + a swallowed `removeChild`). Root cause: NativeCanvas's `root.unmount()` is **synchronous by design** — [NativeCanvas.test.tsx](../playground/src/visualization/__tests__/NativeCanvas.test.tsx) asserts the container is empty *immediately* after unmount and `flushSync` exists so adapter telemetry sees sync mount/unmount. Deferring it (the only silencer) breaks that contract + tests, and the HANDOVER 2026-05-2x tripwire warns deferral reintroduces duplicate-`createRoot` warnings. Page functions correctly; it's dev-build noise. Left as-is on purpose.

**Tripwires:**
- **Warehouse permanent-failure is memoized for 60 s** (`warehouseFailedLog`, server.js). A 4xx start failure short-circuits subsequent calls with zero network for that window — by design (kills the 5-min hang). Transient 5xx/socket errors are NOT memoized.
- **`isBlockedInlineHost` blocks only loopback + link-local/metadata, NOT RFC1918** — intentional (on-prem Databricks can be private). Don't broaden to private ranges without checking on-prem deployers.
- **NativeCanvas unmount is synchronous on purpose** — don't "fix" the StrictMode console warning by deferring `root.unmount()`; it breaks the empty-container-after-unmount contract + flushSync telemetry.
...
- **Azure App Service hosting LIVE** at `https://<your-app>.azurewebsites.net` — RG `pulseplay-rg`, **F1 Free** Linux plan `pulseplay-f1`, web app, Node 20, startup `node proxy/server.js`, `STATIC_DIR=playground/dist`, HTTPS-only. Provisioned via a Node ARM script (device-code → ARM REST) because `az` is Norton-TLS-blocked and `azd` isn't installed; the **Azure MCP is authenticated** (read + app-settings only, can't create). Cred-free build deployed + smoked anonymously, then **Entra Easy Auth** enabled (single-tenant, require-auth) → anonymous now **401 (org-only)**, then full `config.json` build redeployed behind the gate. Power BI Q&A + embed-token both proven via a throwaway local proxy earlier.
...
- **Service principal created** via [scripts/create-pbi-service-principal.mjs](../scripts/create-pbi-service-principal.mjs) (device-code → Graph). App `pulseplay-powerbi-sp`, clientId `<sp-client-id>`, secret expires **2026-11-29**. Tenant `<dev-tenant-guid>` (dev tenant).
...
**Connection result.**
- Confirmed the existing `powerbi-dwd` profile is wired to workspace `<dev-workspace-guid>` and semantic model `<salesperf-dataset-guid>`.
...
**Subscription found.** `Azure subscription 1` is enabled. Subscription ID: `<dev-subscription-id>`. Tenant ID: `<dev-tenant-guid>`.
