...
- [x] **Ask Pulse label + unified surface-tabs proposal** (2026-05-17, shipped/proposed) — Renamed the visible Pulse `Chat` surface to **Ask Pulse** across the viewer tab, setup wizard, Settings AI/Preferences selectors, and format-pane strings while keeping internal `chat` keys stable. Added Rajesh's [Proposed_Preset_Templates.pdf](Proposed_Preset_Templates.pdf) and an AGENT_SYNC proposal for unified surface tabs: `AI Insights | Ask Pulse | BI Viz`, future fused `AI/BI Insights`, BI-only, AI-Insights-only, and Ask-Pulse-only presets. No BI architecture changed yet; Claude/Rajesh review is needed before turning BI into a peer surface tab. Validation: playground lint, full playground **552/552**, diff-check.
