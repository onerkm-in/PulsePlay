...
- [x] **KPI delta cues respect lower-is-better metrics** (2026-05-17, shipped) — Return Rate-style KPI tiles now preserve the raw numeric delta (`+0.4pp`) while rendering the business-performance cue as red/down when `higherIsBetter: false`. Overall tile status and delta tone are separated, so an amber/watch tile can still show a red negative delta cue. Validation: focused renderer **4/4**, lint clean, full playground **572/572**, build clean, browser smoke at `http://127.0.0.1:5173/`.
