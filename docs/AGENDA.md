...
- [x] **Pulse-mode BI source row read-only** (2026-05-16, shipped) — The Pulse AI pane no longer exposes a second BI Tool dropdown. It shows the current BI source/readiness summary and sends changes to Settings › Setup, preserving one configuration source of truth. Validation: playground lint, focused viewport controls **19/19**, full playground **503/503**, build, diff-check.
...
- [ ] **Databricks Vector Search adapter spike** — first enterprise provider because Unity Catalog is the internal platform fit; keep interface provider-agnostic.
