...
- [ ] **G2.5 - Databricks source-ref contract** - define typed Databricks source refs for Genie Spaces, Metric Views, UC Functions, UC Views, and tables; thread `sourceRef` into renderable result envelopes; keep source execution proxy/pack-side and out of the native adapter. ~1-1.5 days.
...
- [ ] **G4-G6 - Native canvas, author switch, fusion-lite** - ECharts canvas MVP, BI surface mode (`auto/native/vendor`) in the picker, then canonical native T2 fusion-lite (chart + docked commentary cards). ~2-4 days total.
