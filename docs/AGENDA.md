...
- [x] **Unified BI Viz same-canvas tab + metric cue cleanup** (2026-05-18, working tree) — `enabledComponents="mix"` now treats `AI Insights`, `Ask Pulse`, and `BI Viz` as peer surfaces in one primary canvas: clicking `BI Viz` swaps to the BI pane without focused-pane mode or `?focus=bi`, and the BI surface includes a compact switcher back to AI Insights / Ask Pulse. KPI delta rendering now keeps lower-is-better unfavorable deltas semantically red/down even when the delta text contains a nested physical up glyph like `+0.4pp (▲ +6.3%)`. Validation: focused viewport + renderer tests **23/23**, playground lint clean.
