...
- [ ] **AI briefing redesign — Phases C + D** (2026-05-18, queued). Rajesh's design direction for the Pulse AI Insights production surface. **Phase A (inline pill semantic cue consistency)** and **Phase B (section label renames)** landed in `1e04a31` + `e87bae0`. Additional "sugar candy" design direction landed as canonical doc + proof slice in `99292ac` ([AI_BRIEFING_DESIGN_DIRECTION.md](AI_BRIEFING_DESIGN_DIRECTION.md) + section stagger + press feedback + status badge pop, all reduced-motion-guarded). Remaining: **Phase C — toolbar noise reduction** (keep surface switcher + Adjust + status; move copy/code/diagnostics to overflow); **Phase D — information hierarchy + visual calming** (top Executive Brief 60/40 with KPI Snapshot, middle KPI + What Changed, lower Next Best Actions made more prominent, fewer nested containers). Both phases inherit the sugar-candy direction. Tripwires: do NOT reintroduce permanent BI pane; do NOT make BI Viz focused-pane default; do NOT change validator or sandbox; do NOT switch ECharts to full bundle; internal section IDs stay (display-only rename); one accent moment per screen; reduced-motion guards mandatory.
