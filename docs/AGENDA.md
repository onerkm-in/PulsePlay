...
- [x] **Error handling strategy and no-panic failure contract** (2026-05-17, docs/research) — Added [ERROR_HANDLING_STRATEGY.md](ERROR_HANDLING_STRATEGY.md) after multi-agent scan across frontend/BI adapters, proxy/API routes, Databricks/connectors, and source-backed modern error handling. Recommendation: RFC 9457 Problem Details + PulsePlay extensions, support-code correlation, category taxonomy, viewer-safe copy, operator detail/runbook links, and tests that prevent raw `err.message` regressions. Brutal-honest P0s: `/responses-agent/*` middleware gap, malformed JSON/global error envelope gap, raw `err.message` leaks in older connector routes, and Databricks OAuth error normalization gap. Runtime code not changed yet.
