...
- **All five runtime dependency trees are clean at their gate** (`proxy`, `playground`, `enablers/desktop`, `enablers/pulse-pbi` at high; `enablers/pulse-pbi-gn` at critical — named exemption below). The sweep that established this found real advisories in shipping code, including a `http-proxy-middleware` CRLF field-injection + Host-header routing bypass in the desktop enabler. (`pulse-pbi-gn` was missing from Dependabot and the first audit sweep entirely — enabling the dependency graph exposed 20 of the repo's 22 open alerts in that one uncovered package.)
- **One caveat npm's dev/prod split hides, stated rather than buried:** `xlsx` sits in the playground's devDependencies but **ships in the production bundle** as a lazy chunk (Vite bundles what is imported, regardless of section), and is a direct runtime dep of the `pulse-pbi-gn` visual. Its two high advisories (prototype pollution, ReDoS) are **read-path** — triggered by parsing crafted workbooks — and PulsePlay's usage is **write-only** (`XLSX.write` of our own data; no `XLSX.read` of user files anywhere in src), so the trigger path does not exist in the product today. There is no npm-registry fix (SheetJS stopped publishing there at 0.18.5). Real fix tracked as AGENDA `DEP-XLSX`; do not add an `XLSX.read` call anywhere without doing that migration first.
