...
| proxy-pbi-rls-failclosed | 0.1 | 2026-06-04 (commit `542743a`): PBI semantic-model RLS is now fail-closed on `/powerbi/qna/embed-token` and `startPowerBiConversation` (`/powerbi/conversations/start`), mirroring `/assistant/embed-token/powerbi` — 401 when RLS is configured but no server-side user identity resolves; OBO `userAssertion` path for identified users. Proxy-only change (`_resolvePowerBIIdentities` + new `_extractUserAssertion`); 2 jest tests. Logged here per the Desktop Cascade Checklist (changes to proxy routes/auth must be recorded). | Pulse PBI **N/A** (custom visual — no proxy bundle; does not call the PBI semantic-model routes). Desktop inherits **automatically** through the byte-for-byte package-time proxy copy (`scripts/package-win.mjs` `copySnapshot()` wipes `out/` then copies fresh from `repoRoot/proxy` — verified; the `out/snapshot` artifact is gitignored and regenerated). No re-port needed, but a **re-package is required to ship it** (existing EXEs predate the fix). OBO success path (RLS enforced for an identified user) is NOT live-verified — no IdP + RLS dataset in this rig. |
