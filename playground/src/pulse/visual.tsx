...
// TREND / FLAT_GLYPH / FLAT_WORD / MEAS_UNIT are deliberately NOT imported:
// they exist only to compose INLINE_REGEX, which now lives with them. Importing
// them here would be dead weight and would re-couple this file to the grammar's
// internals rather than its result.
import {
    isMeasurementNumber,
    isThresholdContext,
...
function App(props: AppProps) {
    ...
    // Per-surface connector overrides were REMOVED for the 2026-07-24 demo
    // (multipane/surfaceConnectors.ts became an always-null seam) and the dead
    // machinery around it — event listeners for an event nothing fired, memos
    // re-reading a permanent null — was deleted 2026-07-28 (DEBT_REGISTER D6).
    // Every surface talks through the single shared connector. If per-surface
    // binding returns, rebuild it from the paneConnectors foundation, not here.
    const askPulseClient = activeClient;
    ...
    const connectionContextKey = [
        ...
        // Historical segment: the removed per-surface override (always ""). Kept
        // as a literal so existing saved chat-state keys still match — dropping
        // the element would change the key and orphan every user's chat history.
        "",
    ].join("|");
    useEffect(() => {
    ...
    return (
        ...
        >
                        ...
                        <div className="gn-header-run-state">
                            ...
                            */}
                            ...
                            <div className="gn-insights-meta">
                                ...
                                {/* 2026-06-03 — theme toggle (sun/moon). Replaces the ⚙ Settings
                                    gear (removed for redundancy per the finalized spec). Flips
                                    darkMode in the genie settings store + fires the change event
                                    so both the native surfaces (themeSync) and the Workbench
                                    re-theme. */}
                                <button
                                    type="button"
                                    className="gn-pane-action-btn"
                                    title={props.settings.darkMode ? "Switch to light theme" : "Switch to dark theme"}
                                    aria-label={props.settings.darkMode ? "Switch to light theme" : "Switch to dark theme"}
                                    aria-pressed={props.settings.darkMode}
                                    onClick={() => {
                                        try {
                                            const k = "pulseplay:visual-settings:genieSettings";
                                            const ex = JSON.parse(localStorage.getItem(k) || "{}");
                                            ex.darkMode = !props.settings.darkMode;
                                ...
                                >
                                    {props.settings.darkMode ? (
                                        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="4" /><path d="M12 2v2M12 20v2M4.9 4.9l1.4 1.4M17.7 17.7l1.4 1.4M2 12h2M20 12h2M4.9 19.1l1.4-1.4M17.7 6.3l1.4-1.4" /></svg>
                                    ) : (
                                        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z" /></svg>
                                    )}
                                </button>
            ...
            {/* 2026-06-03 — provenance footer moved to the HOST level (App.tsx) so it
                shows consistently on EVERY screen (AI Insights / Ask Pulse / Dashboard),
                not just the pulse Workbench. Context lives there only. */}
                            ...
                            {insightsCompareMode && props.settings.multiSpaceEnabled && activeSpaces.length > 1 ? (
                                ...
                                <div className="gn-msg gn-msg--assistant">
                                    <div className="gn-bubble">
                                        ...
                                        {/* 2026-06-02 — the "No status colors · pick a preset" banner is
                                          * suppressed entirely. It was an unbidden nag that re-appeared on
                                          * every briefing without author-configured metric-direction rules
                                          * (flagged repeatedly by Rajesh). Metric-direction presets remain
                                          * configurable in Settings → metric directions; the briefing
                                          * itself already conveys direction via its ▲/▼ · pp deltas. The
                                          * ColorRulesBanner component is retained for that Settings path. */}
