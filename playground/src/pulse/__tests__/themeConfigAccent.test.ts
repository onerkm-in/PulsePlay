...
// dark-canonical steel #5b9bd5, leaving low-contrast accent text/links. The resolver
...
describe("buildAppThemeVars — dark accent (A3)", () => {
    ...
    it("uses the dark-canonical accent (#5b9bd5) for a built-in preset in dark mode", () => {
        const vars = buildAppThemeVars(defaultTokens, { dark: true });
        expect(vars["--pp-accent"]).toBe("#5b9bd5");
        expect(vars["--gn-accent"]).toBe("#5b9bd5");
        // derived family tracks the dark accent, not the light preset value
        expect(vars["--pp-accent-soft"]).toContain("91, 155, 213");
    });
