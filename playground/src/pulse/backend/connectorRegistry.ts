...
const FOUNDATION_MODEL_NOUN = { single: "Foundation Model endpoint", plural: "Foundation Model endpoints" };
const FOUNDATION_MODEL_DESCRIPTOR: ConnectorDescriptor = {
    id: "foundation-model",
    ...
    health: async (config) => {
        ...
        try {
            ...
            if (resp.status >= 200 && resp.status < 300) return { ok: true, detail: "Foundation Model proxy route reachable." };
