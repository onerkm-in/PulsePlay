#!/usr/bin/env node
// Live 10-case headed regression (mix of simple / medium / complex) against
// powerbi-dwd profile. Twin of probe-ask-pulse-live5.mjs, doubled.
...
const EVIDENCE_ROOT = join(REPO, "docs/evidence/powerbi-live10-2026-05-26");
...
async function main() {
    try {
        ...
        await mkdir(OUT_DIR, { recursive: true });
        console.warn(`  [warn] docs/evidence locked; screenshots -> ${OUT_DIR}`);
    }
    ...
    await writeFile(join(OUT_DIR, "summary.json"), JSON.stringify({ pass, total, results }, null, 2));
